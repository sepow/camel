...
from camel.forms import AnswerForm, MultipleChoiceAnswerForm, UserForm, AnswerFormSet
...
class Exercise_ListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        module = Module.objects.get( pk=self.kwargs['pk'] )
        context['module']  = module
        context['exercises'] = TreeNode.objects.filter( node_type="exercise", mpath__startswith=module.code ).order_by('mpath')
...
def exercise(request, pk):
    ...
    context['chapter'] = ex.get_parent_by_type('chapter')
    ...
    # qset = []
    # for qu in questions:
    #     saved_ans = answers.filter(question=qu).first()
    #     if saved_ans:
    #         qset.append(saved_ans)
    #     else:
    #         qset.append(saved_ans)
    # print qset

    answers = Answer.objects.filter(user=request.user, question__in=questions).order_by('pk')
    formset = AnswerFormSet( queryset=answers )
    context['formset'] = formset
    return render(request, 'exercise.html', context)
    # return render_to_response("exercise.html", {
    #     "formset": formset,
    # })
    ...
    # # deal with single forms (preview/save)
#     # retreive current saved answer (if any)
#     if request.method == 'POST':
...
#             #     answer = form.save(commit=False)
#             #     answer.save()
#             if 'edit-answer' in request.POST:
#                 print "EDIT"
#                 print "*************************************************"
...
#                 print "*************************************************"
#             elif 'test-answer' in request.POST:
#                 print "TEST"
#             elif 'preview-answer' in request.POST:
#                 print "PREVIEW"
#             elif 'save-answer' in request.POST:
#                 print "SAVE"
#             elif 'submit-exercise' in request.POST:
#                 print "SUBMIT"
#             else:
#                 print "ANFFODUS"
#         else:
#             print "FORM NOT VALID"
#             context['debug'] = formset.errors

    # formsets
    # AnswerFormSet = modelformset_factory(Answer, form=AnswerForm)
    # formset = AnswerFormSet(queryset=Answer.objects.filter(mpath__startswith=exercise.mpath ).order_by('mpath'))
    # AnswerFormSet = modelformset_factory(Answer, fields=('question', 'user', 'text', 'readonly'))
    # AnswerFormSet = modelformset_factory(Answer, form=AnswerForm)
    # if request.method == 'POST':
