...
# The counter is re-set at each Division
...
#-----------------------------
# TexTree
# children: list of divisions (chapter/section/subsection)
class TexTree(Node):
    def __init__(self, latex_main_file=None):
        ...
        if latex_main_file:
            ...
            self.attributes = parse_preamble(s)
            self.children = parse_document(s)
...
#------------------------------------------------
# translation table from latex cmds/envs to camel classes
#------------------------------------------------
...
def traverse(node, level=0):
    if not node.children:
        ...
        for child in node.children:
            s = 'BEGIN: Node %d (name_str: %s)' % (child.node_number, child.name_str())
            print '----'*level + s
            print '----'*level + 'CHECK: database for object of type %s
            traverse(child, level=level+1)
            s = 'END:   Node %d (name_str: %s)' % (child.node_number, child.name_str())
            print '----'*level + s
#------------------------------------------------
# main
#------------------------------------------------
